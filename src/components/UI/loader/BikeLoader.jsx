import React from "react";
import "./BikeLoader.css";

const BikeLoader = () => {
  return (
    <div>
      <div className="bike">
        <div className="bike__cloud-1"></div>
        <div className="bike__cloud-2"></div>
        <div className="bike__cloud-3"></div>
        <div className="bike__bike">
          <div className="bike__wheel">
            <div className="bike__needle"></div>
            <div className="bike__needle"></div>
            <div className="bike__needle"></div>
          </div>
          <div className="bike__wheel">
            <div className="bike__needle"></div>
            <div className="bike__needle"></div>
            <div className="bike__needle"></div>
          </div>
          <div className="bike__down-tube"></div>
          <div className="bike__tubes">
            <div className="bike__chain"></div>
            <div className="bike__seat-stays"></div>
            <div className="bike__chain-stays"></div>
            <div className="bike__seat-tube"></div>
            <div className="bike__star">
              <div className="bike__pedal"></div>
            </div>
            <div className="bike__seat"></div>
          </div>
          <div className="bike__top-tube"></div>
          <div className="bike__fo"></div>
          <div className="bike__head-tube"></div>
          <div className="bike__helm"></div>
          <div className="bike__lock"></div>
        </div>
        <div className="bike__man">
          <div className="bike__arm">
            <div className="bike__forearm">
              <div className="bike__hand"></div>
            </div>
            <div className="bike__sleeve"></div>
          </div>
          <div className="bike__back-leg">
            <div className="bike__shin">
              <div className="bike__skin"></div>
              <div className="bike__ked"></div>
            </div>
          </div>
          <div className="bike__butt"></div>
          <div className="bike__front-leg">
            <div className="bike__shin">
              <div className="bike__skin"></div>
              <div className="bike__ked"></div>
            </div>
          </div>
          <div className="bike__shirt">
            <div className="bike__collar"></div>
          </div>
          <div className="bike__arm">
            <div className="bike__forearm">
              <div className="bike__hand"></div>
            </div>
            <div className="bike__sleeve"></div>
          </div>
          <div className="bike__head">
            <div className="bike__eye"></div>
            <div className="bike__eye"></div>
            <div className="bike__whisker"></div>
            <div className="bike__nose"></div>
            <div className="bike__month"></div>
            <div className="bike__whisker"></div>
            <div className="bike__cap">
              <div className="bike__peak">
                <div className="bike__peak-parts"></div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default BikeLoader;
